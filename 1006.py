...
class RSIStrategy(bt.Strategy):
    ...
    params = (
        ...
        ('buy_score_threshold', 60),     # 买入评分阈值（满分100）
        ('sell_score_threshold', 60),    # 卖出评分阈值（满分100）
    )
    ...
    def calculate_position_ratio(self):
        ...
        def expo_func_lower(x):
            return (-(x-self.params.rsi_lower)/self.params.rsi_lower)**1.2

        def expo_func_upper(x):
            return ((x-self.params.rsi_upper)/self.params.rsi_lower)**1.2
    ...
    def calculate_buy_score(self):
        ...
        if self.rsi[0] < self.params.rsi_lower:
            # RSI越低，分数越高
            rsi_score = 40 * (self.params.rsi_lower - self.rsi[0]) / self.params.rsi_lower
            score += rsi_score
        ...
        # 短期趋势：价格>MA5 (+12分)
        if close_price > self.ma5[0]:
            ma_score += 12
        # 中期趋势：MA5>MA20 (+12分)
        if self.ma5[0] > self.ma20[0]:
            ma_score += 12
        # 长期趋势：MA20>MA60或MA60斜率>=0 (+11分)
        if self.ma20[0] > self.ma60[0] or self.ma60[0] >= self.ma60[-1]:
            ma_score += 11
    ...
    def calculate_sell_score(self):
        ...
        if self.rsi[0] > 60:  # RSI进入高位区域(60-100)
            # RSI越高，分数越高
            rsi_score = 40 * (self.rsi[0] - 60) / 40  # 60分得0分，100分得40分
            score += rsi_score
            details['RSI'] = f"+{rsi_score:.1f} (RSI={self.rsi[0]:.1f})"
        elif self.rsi[0] > 50:  # RSI在50-60之间，给少量分数
            rsi_score = 10 * (self.rsi[0] - 50) / 10
            score += rsi_score
            details['RSI'] = f"+{rsi_score:.1f} (RSI={self.rsi[0]:.1f})"
        else:
            details['RSI'] = f"+0 (RSI={self.rsi[0]:.1f})"
        ...
        if close_price < self.ma20[0] * 1.02:  # 价格在MA20附近或下方
            ma_score += 10
    ...
    def next(self):
        ...
        # if rsi<30 and volume.. and moving average ..
        # rsi - 40, ma -30, volume-30 --->
        ...
        if self.position:
            ...
            if sell_score >= self.params.sell_score_threshold:
                size = self.calculate_position_size(False)
                # self.sell(size=size)
...
def plot_trades(data, strategy, stock_name):
    ...
    ax3.bar(data.index, data['volume'], color=colors, alpha=0.5, width=0.8)
    ax3.plot(data.index, volume_ma, color='blue', linewidth=2, alpha=0.8, label='成交量MA20')
    ax3.plot(data.index, volume_ma * 1.2, color='orange', linewidth=1.5,
    ...
    ax3.set_ylabel('成交量', fontsize=11)
