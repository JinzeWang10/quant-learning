...
_记得平衡正负样本比例：class_weight='balanced': 自动平衡正负样本权重_

**训练数据**:

- 股票交易中的 train_test_split，因为我们的训练数据中不能包含任何未来数据。
  训练：2022-01-01 至 2025-11-30 随机抽取 80%的数据作为训练 ---> 会抽到 2025-06-30 至 2025-11-30 时间范围内的数据 ---> 他会在回测中表现比预期的好
  回测：2025-07-01 至 2025-11-30
...
1. input data 选取：可以选择更相近股票，训练更”专业“的模型。
2. model(params)调优： -换一个模型？（xgboost，neural network,transformers） -调整模型参数：depth,class_weight,... -买入和卖出可以分成两个模型。最好能训练一个专门的模型预测下跌。
3. model input features + output label
   - 是否有些指标没意义，可以删？指标之前相关性太强？有些指标没考虑到？ ---- feature selection 特征选择
   - 非技术指标：情绪，讨论度，行业景气度，大盘，关联板块技术指标
   - workable practice:一开始尽可能多的加入 features，通过 feature seleciton 的方法不断降维，筛选出真正重要的。
4. model output label
   - 现在是 5 日内涨幅>3%。一日内涨幅？多几个模型，分别预测 2 日内涨幅，5 日内涨幅，10 日内涨幅。买入的时候结合多个模型去判断。
   - 还可以加一个下跌 label 做预测。
   - 从二元分类，变成多元分类。比如 5 日内涨幅>3%;5 日内跌幅>3%;5 日内波动[-3%,3%]
5. 各种 threshold 调整：
   - 超过 60%买入，低于 50%卖出？
   - 各种技术指标的参数
6. 仓位控制：
