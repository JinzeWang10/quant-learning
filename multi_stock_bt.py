    ...
    def calculate_buy_score(self, data_name):
        ...
        if rsi < self.params.rsi_lower:
            if rsi > 25:
                ...
                rsi_score=50
            # RSI越低，分数越高
            details['RSI'] = f"+{rsi_score:.1f} (RSI={rsi:.1f})"
        else:
            details['RSI'] = f"+0 (RSI={rsi:.1f})"
        details['RSI'] = rsi
        ...
        ma_score = 0
        # 短期趋势：价格>MA5
        if indicators['ma5'][0] >= indicators['ma20'][0] and indicators['ma5'][-1] <= indicators['ma20'][-1]:
            ma_score += 15
        # 长期趋势：MA20>MA60或MA60斜率>=0 
        if indicators['ma20'][0] > indicators['ma60'][0] and indicators['ma20'][-1] <= indicators['ma60'][-1]:
            ma_score += 10
        score += ma_score
        ...
        # === 过滤器3: 成交量放量（权重25分） ===
        volume_score = 0
        volume_ratio = data.volume[0] / indicators['volume_ma'][0] if indicators['volume_ma'][0] > 0 else 0
